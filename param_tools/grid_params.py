...
class description:
    def __init__(self,ORGANIC_FRACT=False, SPATIAL_FROST=False, SPATIAL_SNOW=False,
                 EXCESS_ICE=False, JULY_TAVG_SUPPLIED=False, BLOWING_SNOW=False,
                 GLOBAL_LAI=False):
        ...
        self.snow_param = {'cellnum':'Grid cell number (should match numbers assigned in soil parameter file)',
...
def main():
    ...
    units/descriptions are from params.py.  Output is to a netcdf containing all
    the parameterfiles (default name is params.nc)
    """
    grid_file, soil_file, snow_file, veg_file, vegl_file, out_file = process_command_line()
    ...
    print('completed grid_parms.main(), output file was: {}'.format(out_file))
# -------------------------------------------------------------------- #
...
def make_grid(grid_file, soil_file, snow_file, veg_file, vegl_file,
    ...
    to exclude areas in the ocean.  Finally, if the nc_file = 'any_string.nc', a netcdf file
    be written with the parameter data, if nc_file = False, the dictionary of grids is returned.
    """
...
def calc_grid(lats, lons, decimals=4):
    ...
    print('found %s unique lons' %len(ulons))
    lon_step, lon_count = stats.mode(np.diff(ulons))

    ulats = np.sort(np.unique(lats.round(decimals=decimals)))
    print('found %s unique lats' %len(ulats))
    lat_step, lat_count = stats.mode(np.diff(ulats))

    # check that counts and steps make sense
    if lon_step != lat_step:
        print('WARNING: lon_step ({}) and lat_step ({}) do not match'.format(lon_step, lat_step))
    if lat_count/len(ulats)<0.95:
        print('WARNING: lat_count of mode is less than 95% ({}%) of len(lats)'.format(lat_count/len(ulats)))
    if lon_count/len(ulons)<0.95:
        print('WARNING: lon_count of mode is less than 95% ({}%) of len(lons)'.format(lon_count/len(ulons)))

    if lats.min()<-55 and lats.max()>70:
        # assume global grid
        print('assuming grid is meant to be global...')
        target_grid['longitude'] = np.linspace(-180+lon_step[0]/2, 180-lon_step[0]/2, 360/lon_step[0])
        target_grid['latitude'] = np.linspace(-90+lat_step[0]/2, 90-lat_step[0]/2, 180/lat_step[0])
    else:
        target_grid['longitude'] = np.arange(lons.min(), lons.max()+lon_step[0], lon_step[0])
        target_grid['latitude'] = np.arange(lats.min(), lats.max()+lat_step[0], lat_step[0])

    y, x = latlon2yx(lats, lons, target_grid['latitude'], target_grid['longitude'])
    ...
    for i, name in enumerate(in_dicts):
        ...
        for var in mydict:
            if mydict[var].ndim == 1:
                ...
                out_dict[var] = np.ma.zeros((steps, ysize, xsize)) + FILLVALUE_F
...
def write_netcdf(myfile, target_attrs, target_grid,
    ...
    Reads attributes from params.py and from targetAtters dictionary read from grid_file
    """
    ...
    for var, data in soil_grid.iteritems():
        ...
        if data.ndim == 1:
            v = f.createVariable(var, NC_DOUBLE, ('nlayer', ), fill_value=FILLVALUE_F)
            v[:] = data
            ...
            v = f.createVariable(var, NC_DOUBLE, layer_dims, fill_value=FILLVALUE_F)
            v[:, :, :] = data
        else:
    ...
    if snow_grid:
        ...
        snowbnds = f.createDimension('snow_band', snow_grid['AreaFract'].shape[0])
        ...
        for var, data in snow_grid.iteritems():
            ...
            if data.ndim == 2:
                ...
                v = f.createVariable(var, NC_DOUBLE, snow_dims, fill_value=FILLVALUE_F)
                v[:, :, :] = data
            else:
    ...
    if veg_grid:
        ...
        for var, data in veg_grid.iteritems():
            ...
            if veg_grid[var].ndim == 2:
                ...
                v = f.createVariable(var, NC_DOUBLE, mycoords, fill_value=FILLVALUE_F)
                v[:, :, :] = data
        ...
        if veglib_dict:
            ...
            for var, data in veglib_dict.iteritems():
                if data.ndim == 1:
                    v = f.createVariable(var,NC_DOUBLE,('veg_class', ), fill_value=FILLVALUE_F)
                    v[:] = data
                elif data.ndim == 2:
                    v = f.createVariable(var,NC_DOUBLE, ('veg_class','month'), fill_value=FILLVALUE_F)
                    v[:,:] = data
                else:
...
def veg(veg_file, soil_dict, maxRoots=3, vegClasses=11,
    ...
    """
    Read the vegetation file from vegFile.  Assumes max length for rootzones and
    vegclasses.  Also reorders data to match gridcell order of soil file.
    """
